	...
	private async void GenerateLine(Vector2 endPos)
	{
		...
		if (ChainedEnemies.Count <= _chainCount && IsInstanceValid(PathfindingManager.instance.LevelTilemap) && endPos != TargetPosition.Normalized() * (Stats[TowerStat.Range] * PathfindingManager.instance.LevelTilemap.TileSet.TileSize.X / 10f))
		{
			...
			if (halvedStats[TowerStat.Damage] > 0.1f && nextEnemy != null)
				ChainData.InstantiateProjectile(halvedStats, _hitEnemyPosition, _hitEnemyPosition.DirectionTo(nextEnemy.GlobalPosition).Angle() + Mathf.Pi / 2f, ChainedEnemies, this);
		}
	}
